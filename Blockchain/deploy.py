...
def spawnContainer(type_of_container, orderer_flag, container_number):
    ...
    if type_of_container == "load_balancer":
        container = client.containers.run(image=image_to_run, detach=True, network="blockchain", name=container_name, hostname=container_name, ports={'8080/tcp': 80}, environment=["CUSTOM_PORT=8080"], volumes={'/var/run/docker.sock': {'bind': '/var/run/docker.sock', 'mode': 'rw'}, '/home/blockchain/logs/': {'bind': '/usr/src/app/logs', 'mode': 'rw'}})

    else:
        container = client.containers.run(image=image_to_run, detach=True, network="blockchain", name=container_name, hostname=container_name, environment=["CUSTOM_PORT=80"], volumes={'/var/run/docker.sock': {'bind': '/var/run/docker.sock', 'mode': 'rw'}, '/home/blockchain/logs/': {'bind': '/usr/src/app/logs', 'mode': 'rw'}})
